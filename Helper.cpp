...
HCNode* build_tree(unordered_map<char, int> occur_map) {
    priority_queue<HCNode*, vector<HCNode*>, HCNodePtrComp> heap;
    for (auto& pair : occur_map) {
        heap.push(new HCNode(pair.second, pair.first));
    }
    while (heap.size() != 1) {
        HCNode* c1 = heap.top();
        heap.pop();
        HCNode* c0 = heap.top();
        heap.pop();
        HCNode* p = new HCNode(c0->count + c1->count, 255);
        c0->p = p;
        c1->p = p;
        p->c0 = c0;
        p->c1 = c1;
...
void serialize(HCNode* root, FancyOutputStream& fout) {
    if (root == nullptr) return;
    if (root->symbol != 255) {
        fout.write_byte(root->symbol);
        fout.write_byte(' ');
        fout.write_int(root->count);
        fout.write_byte(' ');
        cout << root->symbol << " " << root->count << " ";
    }
    serialize(root->c0, fout);
    serialize(root->c1, fout);
}

void get_codes(HCNode* root, unordered_map<char, string>& codes, string prev_code) {
    if (root == nullptr) return;
    if (root->symbol != 255) codes[root->symbol] = prev_code;
    get_codes(root->c0, codes, prev_code + "0");
    get_codes(root->c1, codes, prev_code + "1");
}

void recurse_clear(HCNode* root) {
    if (root == nullptr) return;
    recurse_clear(root->c0);
    recurse_clear(root->c1);
    delete root;
}
